...
(async()=>{try{
  ...
  const base = get("DEMO_MAIL_API_URL") || get("DEMO_STALWART_API_URL") || (get("STALWART_URL")? get("STALWART_URL").replace(/\/$/,"")+"/api" : "https://mailqa.workinpilot.cloud/api");
  const token = get("DEMO_MAIL_API_TOKEN") || get("STALWART_API_KEY_AUTH_BEARER_TOKEN");
  const intDom = get("DEMO_INTERNAL_EMAIL_DOMAIN","mailqa.workinpilot.cloud");
